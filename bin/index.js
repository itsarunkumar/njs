...
//modules
import { getAppPathChoice, appPathDoesNotExist } from "../src/appPath.js";
import { askForReplaceConfirmation } from "../src/replacePrompt.js";
// Suppress deprecation warning
process.env.NODE_OPTIONS = "--no-deprecation";
const program = new Command();
//starting
program.command("start").description("this is cli tool for nextjs").action(() => {
    console.log(chalk.green("Njs CLI is a CLI tool for Next.js. Its purpose is to create a project and populate it with predefined files"));
});
...
program
    ...
    if (fs.existsSync(projectPath)) {
        const { replace } = await askForReplaceConfirmation(projectName, appPathChoice.appPath);
        if (!replace) {
  ...
  export default function ${projectName}Layout({children}) {
    ...
    setTimeout(() => {
        ...
        spinner.succeed(chalk.green(`Project "${projectName}" created with predefined ${type} files in the '${appPathChoice.appPath}' directory.`));
    }, 2000);
});
//creating component api
program.command("create-api <projectName>").description("Creates an api folder and populates it with predefined files").action(async (projectName) => {
    const rootDirectory = process.cwd();
    const appPathChoice = await getAppPathChoice();
    const appDirectory = path.join(rootDirectory, appPathChoice.appPath);
    const projectPath = path.join(appDirectory, projectName);
